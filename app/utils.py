...
def img_arr_surfs_to_one_arr(img_arr: NDArray[uint8] | None) -> tuple[NDArray[uint8] | None, int, int, int]:
    """
    Преобразование трехмерного массива, содержащего значения трех цветовых компонент в один двумерный массив.
    Компоненты соединяются по горизонтали бок о бок. Монохромное изображение остается без изменений.

    Parameters
    ----------
    img_arr : NDArray[uint8] | None
        массив, содержащий значения цветовых компонент для каждого пикселя изображения
    
    Returns
    -------
    tuple[NDArray[uint8] | None, int, int, int]
        one_arr : NDArray[uint8] | None
            двумерный массив, полученный из входного трехмерного массива
        red_width : int
            ширина красной цветовой плоскости (0 - если монохромное изображение)
        green_width : int
            ширина зеленой цветовой плоскости (0 - если монохромное изображение)
        blue_width : int
            ширина синей цветовой плоскости (0 - если монохромное изображение)
    """
...
def one_arr_to_img_arr_surfs(one_arr: NDArray[uint8], red_width: int=0, green_width: int=0, blue_width: int=0) -> NDArray[uint8] | None:
    """
    Восстановление трехмерного массива, содержащего значения трех цветовых компонент для каждого пикселя изображения,
    из одного двумерного массива, в котором цветовые компоненты соединены по горизонтали бок о бок.
    Монохромное изображение остается без изменений.

    Parameters
    ----------
    one_arr : NDArray[uint8]
        двумерный массив, содержащий значения цветовых компонент
    red_width : int=0
        ширина красной цветовой плоскости (0 - если монохромное изображение)
    green_width : int=0
        ширина зеленой цветовой плоскости (0 - если монохромное изображение)
    ...
    -------
    NDArray[uint8] | None
        восстановленный трехмерный массив
    """
